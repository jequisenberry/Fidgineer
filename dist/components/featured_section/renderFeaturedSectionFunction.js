...
export function buildFeaturedCategoryData(categoryData) {
    const featuredByCategoryData = categoryData.map(category => {
        const featuredItems = category.products.filter(item => category.featuredIds.includes(item.id));
        return Object.assign(Object.assign({}, category), { products: featuredItems });
    });
    return featuredByCategoryData;
}
export function renderCategoryShowcase(featuredCategory) {
    ...
    const randomIndex = Math.floor(Math.random() * featuredCategory.products.length);
    ...
    categoryContainer.append(renderPrimaryFeatured(featuredCategory, randomIndex), cardContainer, renderCTAButton("Show More", "/"));
    // Append to containner
    return categoryContainer;
}
function renderPrimaryFeatured(category, randomIndex) {
    const featuredProduct = category.products[randomIndex];
    ...
    // Category info
    categoryName, categoryDescription, dividerLine, 
    // Featured product info
    productName, productDescription, button);
